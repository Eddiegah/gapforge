 ...
 * 1. Azure OpenAI (most reliable, no daily quota, pays per token from credits)
...
export async function llmCall(
  ...
  // 1. Azure OpenAI — primary (no daily quota, most reliable)
  if (hasAzure) {
    try {
      const text = await callAzureOpenAI(system, prompt, maxTokens);
      return { text, provider: "azure-openai" };
    } catch (err) {
      console.warn("[LLM] Azure OpenAI failed:", err instanceof Error ? err.message : err);
    }
  }
