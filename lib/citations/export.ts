import type { GapCitation, DetectedGap } from "@/lib/gapAI/detectGaps";
...
export function exportToObsidian(gap: import("@/lib/gapAI/detectGaps").DetectedGap, savedId?: string): void {
  const content = [
    `# ${gap.title}`,
    ``,
    ...
    `**Gap Age:** ${(() => { const years = gap.citations.map(c => c.year).filter(Boolean) as number[]; if (!years.length) return 'unknown'; return `${new Date().getFullYear() - Math.min(...years)} years`; })()}`,
    ``,
    `## Description`,
    gap.description,
    ``,
    ...
    gap.whyUnresolved ? `## Why Unresolved\n${gap.whyUnresolved}\n` : '',
    gap.suggestedDirection ? `## Suggested Direction\n> ${gap.suggestedDirection}\n` : '',
    `## Supporting Literature`,
    ...gap.citations.map((c, i) => `${i+1}. [[${c.title}]] - ${c.authors.slice(0,2).join(', ')} (${c.year ?? 'n.d.'}) [${c.url}]`),
    ``,
    `## Metadata`,
    `- Source: GapForge`,
...
export function exportToNotion(gap: DetectedGap): void {
  // redirect to obsidian-export module
  void gap;
}
